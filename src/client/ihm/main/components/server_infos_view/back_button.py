...
class BackButton(Component):
    def __init__(self, pygame_manager: pygame_gui.UIManager) -> None:
        ...
        self.is_last_view_connexion_form = True

    def set_is_last_view_connexion_form(
        self, is_last_view_connexion_form: bool
    ) -> None:
        self.is_last_view_connexion_form = is_last_view_connexion_form
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if event.button == 1:
                if self.gui_element.get_abs_rect().collidepoint(event.pos):
                    if self.is_last_view_connexion_form:
                        self.controller.handle_go_back_to_connection_form()
                    else:
                        self.controller.handle_go_back_to_inscription_form()
